...
class CollectionConfig:
    party_sizes: list[int] = field(default_factory=lambda: [2, 3, 4, 5, 6])
    ...
    try:
        config = AppConfig(
            ...
            collection=CollectionConfig(
                party_sizes=[
                    int(value) for value in collection.get("party_sizes", [2, 3, 4, 5, 6])
                ],
...
def validate_config(config: AppConfig) -> None:
    ...
    elif 60 % config.collection.interval_minutes:
        errors.append("collection.interval_minutes must divide evenly into 60 minutes")
